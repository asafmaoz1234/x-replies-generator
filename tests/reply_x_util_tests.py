...
class TestThreadFilter:
    ...
    def test_op_last_reply_no_replies_needed(self, threads_no_reply):
        ...
        result = filter_threads_with_op_last_reply(threads_no_reply, original_author_id)
        assert len(result) == 0

    def test_other_last_reply_reply_needed(self, threads_needed_reply):
        original_author_id = "1884611255320674304"
        result = filter_threads_with_op_last_reply(threads_needed_reply, original_author_id)
        assert len(result) == 1
        assert result[0]["id"] == "18877408015951544469"

    def test_empty_threads(self):
        result = filter_threads_with_op_last_reply([], "any_id")
        assert len(result) == 0

    def test_no_referenced_tweets(self):
        threads = [{"id": "1", "author_id": "123"}]
        result = filter_threads_with_op_last_reply(threads, "123")
        assert len(result) == 0
